...
def obtain_token():
    ...
    conn = http.client.HTTPSConnection(
        current_app.config["VAULT_HOSTNAME"],
        port=current_app.config["VAULT_PORT"],
        context=context,
    )
    payload = '{\r\n\t"name": "attendance"\r\n}'
    headers = {"Content-Type": "application/json"}
    conn.request("POST", current_app.config["VAULT_LOGIN_URL"], payload, headers)
...
def obtain_data():
    ...
    data = json.loads(response.read().decode("utf8")).get("data").get("data")
    return data
